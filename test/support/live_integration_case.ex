defmodule Jido.Gemini.LiveIntegrationCase do
  @moduledoc false

  use ExUnit.CaseTemplate

  alias Jido.Gemini.{CLI, Compatibility}

  @env_loaded_key {__MODULE__, :env_loaded}

  using do
    quote do
      import Jido.Gemini.LiveIntegrationCase
  ...
  setup _tags do
    ...
    {:ok,
     prompt: live_prompt(),
     cwd: live_cwd(),
     model: env_value("JIDO_GEMINI_LIVE_MODEL"),
     timeout_ms: env_integer("JIDO_GEMINI_LIVE_TIMEOUT_MS", 180_000),
     cli_opts: cli_opts(),
     require_success?: truthy?(System.get_env("JIDO_GEMINI_REQUIRE_SUCCESS"))}
  end

  def live_prompt do
    env_value("JIDO_GEMINI_LIVE_PROMPT") || "Reply with exactly one word: READY"
  end

  def live_cwd do
    env_value("JIDO_GEMINI_LIVE_CWD") || File.cwd!()
  end

  def cli_opts do
    case env_value("JIDO_GEMINI_CLI_PATH") || env_value("GEMINI_CLI_PATH") do
      nil -> []
      path -> [gemini_cli_path: path]
    end
  end
