defmodule Jido.Gemini.Integration.AdapterLiveIntegrationTest do
  use ExUnit.Case, async: false
  use Jido.Gemini.LiveIntegrationCase

  alias Jido.Gemini.Adapter
  alias Jido.Harness.RunRequest

  @integration_skip_reason Jido.Gemini.LiveIntegrationCase.skip_reason()

  if @integration_skip_reason do
    @moduletag skip: @integration_skip_reason
  end

  test "adapter emits a terminal harness event via the real Gemini CLI", ctx do
    attrs =
      %{
        prompt: ctx.prompt,
        cwd: ctx.cwd,
